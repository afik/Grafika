...
void Garis::drawLine(Buffer b, Warna warna)
{
	...
	for (;;){    
		b.setLocation(P1.getX(), P1.getY());
		*(b.getFbp() + b.getLocation()) = warna.B; // Some blue 
		*(b.getFbp() + b.getLocation() + 1) = warna.G; // A little green 
		*(b.getFbp() + b.getLocation() + 2) = warna.R; // A lot of red
		*(b.getFbp() + b.getLocation() + 3) = 0; // No transparency                                                      /* loop */

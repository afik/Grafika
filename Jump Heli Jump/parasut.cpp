...
void Parasut::drawParasut(Buffer buff)
{
	...
	/** Parasut */
	b.setengah_lingkaran(this->getPosition().getX(), this->getPosition().getY()+this->getVelocity(), this->getRadius(), buff, *Warna::shockpink());
	/** end of Parasut*/
	...
	for(i=0; i<3; i++)
	{
		if(i<2)
		{
			...
			g.drawLine(buff, *Warna::shockpink());
		}
		else
		{
			Garis g(taliParasut[i].getX(), taliParasut[i].getY()+this->getVelocity(),
	...
	b.circle(penerjun[0].getX(), penerjun[0].getY()+this->getVelocity(), radius_kepala, buff, *Warna::coklat());
